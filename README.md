...
| expandOnRowDoubleClicked | bool | false |  | The default behavior is to expand the row when the expander button is clicked. `expandOnRowDoubleClicked` allows expanding the row when the an area within the row is double clicked. Requires `expandableRows` be set to true |
