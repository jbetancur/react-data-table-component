...
export function CellBase({ $headCell, $noPadding, headStyle, cellStyle, className, style, ...rest }: CellBaseProps): JSX.Element {
	const baseClass = ['rdt_cellBase', $headCell && 'rdt_cellBaseHead', $noPadding && 'rdt_cellNoPadding', className].filter(Boolean).join(' ');
	return (
		<div
			className={baseClass}
			style={{ ...($headCell ? headStyle : cellStyle), ...style }}
			{...rest}
		/>
	);
}
...
type CellExtendedProps = React.HTMLAttributes<HTMLDivElement> & CellProps & {
	$headCell?: boolean;
	$noPadding?: boolean;
	headStyle?: React.CSSProperties;
	cellStyle?: React.CSSProperties;
...
export function CellExtended({ button, grow, maxWidth, minWidth, width, right, center, compact, hide, allowOverflow, $headCell, $noPadding, headStyle, cellStyle, className, style, ...rest }: CellExtendedProps): JSX.Element {
	...
	const baseClass = ['rdt_cellBase', $headCell && 'rdt_cellBaseHead', $noPadding && 'rdt_cellNoPadding', hideClass, className].filter(Boolean).join(' ');
	return (
		<div
			className={baseClass}
