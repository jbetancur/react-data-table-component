		...
		<div className="relative" ref={ref}>
			...
			{open && (
				<div className="absolute right-0 z-20 mt-1 w-36 bg-white border border-gray-200 rounded-lg shadow-lg py-1 text-sm">
					...
					<button
						onClick={e => { e.stopPropagation(); setOpen(false); onDuplicate(row); }}
						className="w-full text-left px-3 py-1.5 hover:bg-gray-50 text-gray-700"
					>
						Duplicate
					</button>
	...
	return (
		<div className="relative">
			...
			{modal.type === 'edit' && editDraft && (
				<div className="fixed inset-0 z-30 flex items-center justify-center bg-black/20" onClick={() => setModal({ type: 'none' })}>
					<div className="bg-white rounded-xl border border-gray-200 shadow-xl p-6 w-80 space-y-4" onClick={e => e.stopPropagation()}>
						...
						<div className="flex justify-end gap-2 pt-1">
							<button onClick={() => setModal({ type: 'none' })} className="px-3 py-1.5 text-sm text-gray-500 hover:text-gray-700">Cancel</button>
							<button onClick={commitEdit} className="px-3 py-1.5 text-sm bg-brand-600 text-white rounded-md hover:bg-brand-700">Save</button>
						</div>
					</div>
				</div>
			)}
			...
			{modal.type === 'duplicate' && (
				<div className="fixed inset-0 z-30 flex items-center justify-center bg-black/20" onClick={() => setModal({ type: 'none' })}>
					<div className="bg-white rounded-xl border border-gray-200 shadow-xl p-6 w-72 space-y-4" onClick={e => e.stopPropagation()}>
						<h3 className="font-semibold text-gray-900">Duplicate row?</h3>
						<p className="text-sm text-gray-500">A copy of <strong>{modal.row.name}</strong> will be added to the list.</p>
						<div className="flex justify-end gap-2">
							<button onClick={() => setModal({ type: 'none' })} className="px-3 py-1.5 text-sm text-gray-500 hover:text-gray-700">Cancel</button>
							<button onClick={() => commitDuplicate(modal.row)} className="px-3 py-1.5 text-sm bg-brand-600 text-white rounded-md hover:bg-brand-700">Duplicate</button>
						</div>
					</div>
				</div>
			)}
			...
			{modal.type === 'delete' && (
				<div className="fixed inset-0 z-30 flex items-center justify-center bg-black/20" onClick={() => setModal({ type: 'none' })}>
					<div className="bg-white rounded-xl border border-gray-200 shadow-xl p-6 w-72 space-y-4" onClick={e => e.stopPropagation()}>
						...
						<p className="text-sm text-gray-500">This will permanently remove <strong>{modal.row.name}</strong>.</p>
						<div className="flex justify-end gap-2">
							<button onClick={() => setModal({ type: 'none' })} className="px-3 py-1.5 text-sm text-gray-500 hover:text-gray-700">Cancel</button>
							<button onClick={() => commitDelete(modal.row)} className="px-3 py-1.5 text-sm bg-red-600 text-white rounded-md hover:bg-red-700">Delete</button>
						</div>
					</div>
				</div>
			)}
