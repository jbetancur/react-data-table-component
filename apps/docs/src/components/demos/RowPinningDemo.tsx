...
const STATUS_STYLE: Record<Status, React.CSSProperties> = {
	Active: { background: '#dcfce7', color: '#15803d', padding: '2px 10px', borderRadius: 999, fontSize: 12, fontWeight: 600 },
	'On Leave': { background: '#fef9c3', color: '#854d0e', padding: '2px 10px', borderRadius: 999, fontSize: 12, fontWeight: 600 },
	Terminated: { background: '#fee2e2', color: '#991b1b', padding: '2px 10px', borderRadius: 999, fontSize: 12, fontWeight: 600 },
};
	...
	const columns = useMemo((): TableColumn<Employee>[] => [
		{
			...
			cell: r => (
				<div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
					...
					{r.name}
					{pinnedIds.has(r.id) && (
						<span style={{ fontSize: 10, background: '#ede9fe', color: '#6d28d9', padding: '1px 6px', borderRadius: 999, fontWeight: 600 }}>
							pinned
						</span>
					)}
				</div>
	...
	return (
		<div className="space-y-3">
			<p className="text-xs text-gray-500">
				Click the 📌 icon on any row to pin it. Pinned rows stay at the top regardless of how you sort.
				Sort any column to see the behaviour.
			</p>
			<DataTable
				columns={columns}
				data={rawData}
