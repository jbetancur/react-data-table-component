...
interface Row { id: number; name: string; department: string; salary: number; status: string }
...
const columns: TableColumn<Row>[] = [
    ...
    format: r => `$${r.salary.toLocaleString()}`, right: true },
  {
    name: 'Status',
    selector: r => r.status,
    cell: r => <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColor[r.status]}`}>{r.status}</span>,
  },
];
